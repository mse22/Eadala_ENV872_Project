...
# Research question 1:
#a. What is the average nitrogen:phosphorus ratio for each of the six north temperate lakes? Are the lakes phosphorus or nitrogen deficient? 
#b. Have the lakes been within the limiting nutrient criteria over the years? How does the N:P ratio impact the dissolved oxygen in the lakes?
...
Temp.plot <- ggplot(DO.predictors, aes(x = temperature_C, y = dissolvedOxygen, color = lakename)) +
  ...
  ylab("Dissolved Oxygen ()") + # To label the y axis
  labs (color = "Lake Name") + # To label the legend
  ylim(0,30)
print(Temp.plot)
...
Daynum.plot <- ggplot(DO.predictors, aes(x = daynum, y = dissolvedOxygen, color = lakename)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  scale_color_brewer(palette = "Set1", direction = 1) +
  xlab("Day Numbee") + # To label the x axis
  ylab("Dissolved Oxygen ()") + # To label the y axis
  labs (color = "Lake Name") # To label the legend
print(Month.plot)
