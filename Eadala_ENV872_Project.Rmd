...
Table: Chemisty-Physics Dataset Content
...
A question that can be used for further data exploring: Are the lakes phosphorus or nitrogen deficient?
...
Visual to understand if most of the lakes are above or below the Redfield Ratio. In other words, to understand if most of the lakes are phosphorus or nitrogen limited in Figure 5. Dashed line indicates the Redfield Ratio of 7.24:1, which indicates optimal conditions for phytoplankton growth. The lakes above the Redfield Ratio would be phosphorus limied while those below it would be nitrogen limited. 
...
Figure 5 Tells us that most of the Lakes are phosphorus deficient, therefore phosphorus should be the limiting nutrient priortizied or addressed during water management.

Since all the lakes are phosphoshorus deficient, have the water management practices managed to reduce the phosphorus over the years?

```{r fig6, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Nitrogen concentration over years"}
...
TN.years.plot <- ggplot(NitrogenPhosphorus, aes(x = year4, y = tp_ug, color = lakename)) +
  geom_point(alpha = 0.75) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  scale_color_brewer(palette = "Set1", direction = 1) +
  ...
  labs (color = "Lake Name") # To label the legend
print(TN.years.plot) # Gives the N vs P plot; tells us that there has been a decrease in phosphorus concentration over the years. 

```

Figure 6 tells us that there has been a decrease in phosphorus concentration over the years. 
...
Figure 7 presents a visualization of the TP concentrations in Wisconsin lakes data in comparison with the Wisconsin regulatory standard.

```{r fig7, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Phosphorus concenration over years."}
...
Coming the the next part of the research question, figure 8 gives us the visual interpretation of how the N:P ratio affects the dissolved oxygen.

```{r fig8, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen over N:P ratio"}
...
Figure 8 tells us that an increase in the N:P ratio increased dissolved oxygen in the lakes. This is consistent with the theory that controlling phosphorus concentration in phosphorus deficient lakes can can preserve their water quality. However, phosphorus might not be the only factor affecting the dissolved oxygen and eventutally the water quality of water. Therefore, it is important to understand what other variables affect dissolved oxygen, whose depletion indicates eutrophication in many cases.
...
To answer this question, an analysis of covariance (ANCOVA) test was performed.

Since multiple explanatory variables are being considered at a time in the model, it is important to check the model is not over-parameterized. Thereofre, the Akaike's Information Criteria (AIC) was used. 
...
The final set of variables that best predict the dissolved oxygen across a monitoring period are year, day number, temperature, depth, nitrogren concentration and phosphorus concentration. This set of variables explain 81% of variance (Adjusted R-squared = 0.8118, p-vaue < 0.001)

```{r fig9, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Model diagnostic plots."}
...
The following can be said about Figure 9:
...
Given that our assumptions have been met, we can move on to the next step. From the model we got that the intercept is 59.7811180, which is the dissolved oxygen in mg/L, when all the other variables are zero. Therefore, we reject the null hypothesis of no effect of the explanatory variables on the reponse variable. 
...
The temperature decreases the dissolved oxygen (mg/l) in the lakes highly significantly. With an increase in temperature by 1 celcius, the dissolved oxygen is decreased by 0.1370994 mg/l (t = -6.273, df = 900, p < 0.001).
...
*Dissolved oxygen in mg/l = 59.7811180 - 0.0232059xNo.of years - 0.0069474xNo.of days - 0.1370994xtemperature in C - 0.8746669xdepth in m - 0.0013804xTN in µg/l + 0.0076268xTP in µg/l*

Figure 10 - Figure 15 offer visual presentation of the dissolve oxygen against each model variable. 

```{r fig10, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen vs model variable 1"}
...
```{r fig11, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen vs model variable 2"}
...
```{r fig12, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen vs model variable 3"}
...
```{r fig13, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen vs model variable 4"}
...
```{r fig14, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen vs model variable 5"}
...
```{r fig15, echo=FALSE,error=FALSE,message=FALSE,warning=FALSE,fig.cap="Dissolved oxygen vs model variable 6"}
